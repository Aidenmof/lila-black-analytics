...
All modules are in the same folder (flat structure).
...
from filters import render_filters, get_selected_match
from coordinate_mapper import add_pixel_coords, get_minimap_path
from journey_plotter import plot_journeys
from event_markers import add_event_markers, render_event_toggle, NON_MOVE_EVENTS
from heatmap import add_heatmap_overlay, render_heatmap_controls
from timeline import render_timeline, render_event_timeline_chart, render_live_stats
...
with tab_journey:
    ...
    if match_id:
        ...
        st.markdown("---")
        fig = plot_journeys(df=display_df, map_name=selected_map,
                            minimap_path=minimap_path, match_id=match_id,
                            max_timestamp=max_ts)
        if show_events:
            fig = add_event_markers(fig=fig, df=display_df, match_id=match_id,
                                    max_timestamp=max_ts, show_events=show_events)
        st.plotly_chart(fig, use_container_width=True)
        with st.expander("📊 Events per minute"):
            render_event_timeline_chart(display_df, match_id)

with tab_heatmap:
    st.markdown("Heatmaps aggregate **all filtered matches**.")
    if not selected_heatmaps:
        st.info("Enable a heatmap overlay in the sidebar under **Heatmap overlays**.")
    else:
        fig_hm = go.Figure()
        if minimap_img:
            fig_hm.add_layout_image(dict(source=minimap_img, x=0, y=img_h,
                                         xref="x", yref="y", sizex=img_w, sizey=img_h,
                                         sizing="stretch", layer="below", opacity=1.0))
        for hm_type in selected_heatmaps:
            fig_hm = add_heatmap_overlay(fig=fig_hm, df=display_df,
                                          heatmap_type=hm_type, opacity=heatmap_opacity)
...
with tab_stats:
    ...
    with col_a:
        ...
        fig_ec = px.bar(counts, x="Event", y="Count", color="Event",
                        title="Event breakdown",
                        color_discrete_map={"Kill":"#E63946","BotKill":"#FF6B6B",
                            "Killed":"#2D2D2D","BotKilled":"#555555",
                            "KilledByStorm":"#9B59B6","Loot":"#FFD700"}, height=320)
        fig_ec.update_layout(showlegend=False, plot_bgcolor="rgba(0,0,0,0)",
                              paper_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig_ec, use_container_width=True)
    with col_b:
        pt = display_df.groupby("is_bot")["event_type"].count().reset_index()
        pt["Type"] = pt["is_bot"].map({True: "Bot", False: "Human"})
        fig_pt = px.pie(pt, names="Type", values="event_type",
                        title="Human vs Bot event share", color="Type",
                        color_discrete_map={"Human":"#457B9D","Bot":"#E9C46A"}, height=320)
        fig_pt.update_layout(plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)")
        st.plotly_chart(fig_pt, use_container_width=True)
    st.markdown("#### Top 20 players by kills")
    kills_df = (display_df[display_df["event_type"].isin(["Kill","BotKill"])]
                .groupby(["player_id","is_bot"]).size().reset_index(name="kills")
                .sort_values("kills", ascending=False).head(20))
